...
_Last updated: 2026-08-03 (`chore: eslint no-undeclared-envs`)_
...
| `local-rules/envs-order` (`vars-order.js`)               | custom                                              | warn, **autofixable** | keeps `env.d.ts` listing the same variables in the same order as `.env.example`, and keeps `.env.example` itself grouped (site URL, Supabase, Redis/Upstash, AWS, Pusher, then the rest). A name in `.env.example` missing from `env.d.ts` is auto-fixed; a name in `env.d.ts` missing from `.env.example` is only reported - only the owner knows whether to add it back or delete the leftover declaration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `local-rules/no-defined-unused-envs` (`unused-declared-vars.js`) | custom                                       | warn                  | a `ProcessEnv` declaration in `env.d.ts` whose name appears in no `.ts`/`.tsx` file of the repo (bare-name match, not `process.`-prefixed - covers reads through a decrypted object, a destructure, or a template string). The message opens on whether `.env.example` still lists the name, then spells out the five settlements: **1.** used on a website and not in the codebase, the way `GITHUB_CLIENT_ID` lives in the Supabase setup - comment the declaration out and name the website in it, comment its `.env.example` line out the same way, keep the live value as a commented line in `.env.local`. Not a suppression: a suppressed declaration still type-checks, so TS autocomplete keeps offering a name no code reads. Needs no `eslint-disable` either - a commented line is stripped before the parse, so the rule goes quiet on its own; **2.** a library reads it without being handed it (`Redis.fromEnv()` reads `UPSTASH_REDIS_REST_URL`) - keep it, suppress with that call named in the reason; **3.** that same library also takes it as an argument - hand it over explicitly (`createClientComponentClient({ supabaseKey: process.env.X })`) and the warning goes on its own; **4.** read by CI/CD and by a development-only command (`CHROMATIC_PROJECT_TOKEN` in `.github/workflows/chromatic.yml` and `pnpm storybook`) - keep it, suppress with the workflow file named; **5.** literally unused - delete the declaration and its `.env.example` line. Never auto-fixed - 1 comments the line out, 2 and 4 keep it and suppress, 3 swaps the suppression for an argument, 5 deletes it, and nothing in the sources tells them apart                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `local-rules/no-undefined-used-envs`                     | custom                                              | warn                  | the reverse of `no-defined-unused-envs`: `process.env.X` read somewhere in the app where `ProcessEnv` has no declaration for `X`. TypeScript alone won't catch this - `NodeJS.ProcessEnv`'s index signature lets any name through as `string \| undefined` whether or not it's declared. `NODE_ENV` is the one hard-coded skip (Node's own ambient var, never declared here). Suppress a deliberately-undeclared name (a platform-injected or legacy fallback) with `// eslint-disable-next-line local-rules/no-undefined-used-envs -- <reason>`                                                                                                                                                                                                                                                                                                                                                                                                     |
