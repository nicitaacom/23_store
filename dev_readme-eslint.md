...
| `local-rules/no-unused-envs` (`unused-declared-vars.js`) | custom                                              | warn                  | a `ProcessEnv` declaration in `env.d.ts` whose name appears in no `.ts`/`.tsx` file of the repo (bare-name match, not `process.`-prefixed - covers reads through a decrypted object, a destructure, or a template string). Never auto-fixed - only the owner knows whether it's read from outside the sources (CI, a shell script) or genuinely left over                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `local-rules/no-undeclared-envs`                         | custom                                              | warn                  | the reverse of `no-unused-envs`: `process.env.X` read somewhere in the app where `ProcessEnv` has no declaration for `X`. TypeScript alone won't catch this - `NodeJS.ProcessEnv`'s index signature lets any name through as `string \| undefined` whether or not it's declared. `NODE_ENV` is the one hard-coded skip (Node's own ambient var, never declared here). Suppress a deliberately-undeclared name (a platform-injected or legacy fallback) with `// eslint-disable-next-line local-rules/no-undeclared-envs -- <reason>`                                                                                                                                                                                                                                                                                                                                                                                                     |
