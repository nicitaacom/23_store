...
export function Language({ className }: { className?: string }) {
  ...
  return (
    <div className={`relative flex justify-between items-center gap-x-2 border-[1px] border-solid rounded-[4px]
    h-[48px] max-h-[42px] cursor-pointer px-4 py-2 ${className}`}
        ...
        <div className="flex items-center">
          ...
          <h1 className="uppercase">
            {languages.map(language => (
              currentLanguage === language.id && language.code
            ))}
          </h1>
        </div>
      ...
      border-[1px] border-solid border-foreground z-10 
       flex flex-col text-md text-center ${showDropdown
        ...
        {languages.map(language => (
          language.id !== 5
            ? <Link className={`border-b-[1px] border-solid border-foreground
        transition-all duration-[300ms]
            ...
            : <Link className={`border-solid border-foreground
        transition-all duration-[300ms]
         ${isHover
                ? hover === language.id && 'active'
                : currentLanguage === language.id && 'active'}`}
