...
export function AdminModal({ isOpen, onClose, label }: AdminModalProps) {
  ...
  return (
    <ModalContainer className={`w-[100vw] max-w-[500px] tablet:max-w-[650px] 
     ...
     bg-primary rounded-md border-[1px] border-solid border-border-color py-8 overflow-y-scroll transition-all duration-500`}
      ...
      <div className="flex flex-col justify-center items-center w-1/2 mx-auto ">
        ...
        <ul className="flex flex-col tablet:flex-row w-[150%] justify-center mb-8">
          <li><RadioButton label="Add product" inputName="product" onChange={(e) => setProductAction(e.target.value)} defaultChecked /></li>
          <li><RadioButton label="Edit product" inputName="product" onChange={(e) => setProductAction(e.target.value)} /></li>
          <li><RadioButton label="Delete product" inputName="product" onChange={(e) => setProductAction(e.target.value)} /></li>
        </ul>
