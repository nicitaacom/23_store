...
export default async function Home() {
  ...
  const cart_quantity = cart_quantity_response.data?.cart_quantity ?? undefined
  ...
  const cart_products = cart_products_response.data?.cart_products ?? undefined
  ...
  return (
    <div className="text-2xl text-white flex flex-col gap-y-8 justify-between items-center py-12">
      <section className="flex flex-col gap-y-4">
        <Products products={products} cart_quantity={cart_quantity} cart_products={cart_products} />
      </section>
    </div>
  )
}
