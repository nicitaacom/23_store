...
export function FormatImagesForm({ id, imgUrl }: FormatImagesFormProps) {
  ...
  async function saveImages(nextUrls: string[], filesToUpload: ImageListType) {
    ...
    try {
      ...
      const existingCount = nextUrls.filter(u => !u.startsWith("data:")).length
      let uploadIndex = 0
  ...
  function removeImage(index: number) {
    if (index < imgUrl.length) {
      const next = imgUrl.filter((_, i) => i !== index)
    ...
    <ImageUploading
      ...
      {({ onImageUpload, dragProps, isDragging }) => (
        <div className="flex flex-col gap-2">
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-subTitle/70">Images</p>
