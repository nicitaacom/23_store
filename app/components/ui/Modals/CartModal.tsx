...
export function CartModal({ isOpen, onClose, label }: CartModalProps) {
  ...
  async function sendMoney() {
    ...
    try {
      const response = await axios.get(`${baseBackendURL}/coinmarketcap?${convertUSDToETHQuery}&symbol=USD&convert=ETH`)
    ...
    window.ethereum
        ...
        params: [
          {
            ...
            to: process.env.NEXT_METAMASK_ADRESS,
            gasLimit: "0x5028",
      ...
      .then((txHash: string) => {
        router.push(`${baseURL}/payment?status=success`), console.log("txHash - ", txHash)
      })
      .catch((error: Error) => {
      ...
      toast.show(
        ...
        <span className="inline">
          ...
          <Button
            ...
            active="active"
            target="_blank"
            href={`${baseURL}/docs/customer/how-to-install-metamask`}>
            this
          </Button>{" "}
          guide
        </span>,
        ...
        <div className="relative flex flex-col gap-y-8 pb-8 w-full h-full overflow-y-scroll">
          ...
          {userCartStore.products.length > 0 ? (
            <div className="flex flex-col gap-y-4">
                ...
                {userCartStore.products.map(product => (
                  <article
                    className={`flex flex-col laptop:flex-row border-[1px] ${
                      product.on_stock === 0 && "border-warning"
                    }`}
                    key={product.id}>
                        ...
                        <div className="flex flex-col w-full mobile:w-[60%]">
                          <h1
                            className={`text-2xl text-center mobile:text-start truncate ${
                              product.on_stock === 0 && "text-subTitle"
                            }`}>
                            {product.title}
                          </h1>
              ...
              <section className="flex flex-col gap-y-4 gap-x-4 justify-between items-center w-[90%] mx-auto px-4 laptop:px-0">
                    ...
                    <span>
                      {formatCurrency(
                        userCartStore.products.reduce(
                          (totalPrice, product) =>
                            product.on_stock === 0 ? totalPrice : totalPrice + product.price * product.quantity,
                          0,
                        ),
                      )}
                    </span>
                ...
                <div className="grid grid-cols-2 gap-4 laptop:flex flex-row">
                  ...
                  <div>
                    <Button
                      className="flex flex-row gap-x-1 w-full laptop:w-fit"
                      variant="info"
                      disabled={isConnecting}
                      onClick={wallet ? sendMoney : handleConnect}>
                      Metamask
                  ...
                  <form action={`${baseBackendURL}/create-payment?${payPalProductsQuery}`} method="POST">
                    <Button className="flex flex-row gap-x-1 w-full laptop:w-fit" variant="info" type="submit">
                      PayPal
                      <FaPaypal />
                    </Button>
                  </form>
                  <form action={`${baseBackendURL}/create-checkout-session?${stripeProductsQuery}`} method="POST">
                    <Button className="flex flex-row gap-x-1 w-full laptop:w-fit" variant="info" type="submit">
                      Stripe
                      <FaStripeS />
                    </Button>
            ...
            <div className="flex flex-col justify-center items-center">
              <Image src="/empty-cart.png" alt="empty cart" width={512} height={512} />
