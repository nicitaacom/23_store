 ...
 * Read on first use, not at import: the visit-tracking action module is imported while the locale
...
export function getDeviceIdEncryptionKey(): Buffer {
  ...
  if (!keyHex || !DEVICE_ID_KEY_PATTERN.test(keyHex))
    throw Error("DEVICE_ID_ENCRYPTION_KEY must hold 64 hex characters (32 bytes) - generate one with `openssl rand -hex 32`")
