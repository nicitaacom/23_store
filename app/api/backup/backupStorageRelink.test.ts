...
describe("backup Storage URL relinking", () => {
  it("reads a backup bucket and decoded path from a public URL", () => {
    expect(selectBackupStoragePath(`${OLD_PUBLIC_URL}/23_public-images/folder/My%20Image.avif`)).toEqual({
  ...
  it("relinks matching scalar, array and nested JSON values", () => {
    const matchingUrl = `${OLD_PUBLIC_URL}/23_public-images/products/item.avif`
    const nestedUrl = `${OLD_PUBLIC_URL}/23_public-images/mockups/mousepad.png`
    const relinked = relinkBackupStorageValue(
      ...
      new Set(["23_public-images/products/item.avif", "23_public-images/mockups/mousepad.png"]),
      CURRENT_SUPABASE_URL,
    )

    expect(relinked.value).toEqual({
      img_url: [`${CURRENT_SUPABASE_URL}/storage/v1/object/public/23_public-images/products/item.avif`],
      variants: [{ image_url: `${CURRENT_SUPABASE_URL}/storage/v1/object/public/23_public-images/products/item.avif` }],
      personalization: {
        defaultConfig: { mockupUrl: `${CURRENT_SUPABASE_URL}/storage/v1/object/public/23_public-images/mockups/mousepad.png` },
      },
    })
  ...
  it("keeps external, current and missing-path URLs unchanged", () => {
    const currentUrl = `${CURRENT_SUPABASE_URL}/storage/v1/object/public/23_public-images/current.avif`
    const missingUrl = `${OLD_PUBLIC_URL}/23_public-images/missing.avif`
    const externalUrl = "https://images.example.com/storage/v1/object/public/23_public-images/current.avif"
    const value = [externalUrl, currentUrl, missingUrl, "data:image/png;base64,abc"]
    const relinked = relinkBackupStorageValue(
      value,
      new Set(["23_public-images/current.avif"]),
      CURRENT_SUPABASE_URL,
    )
    ...
    expect(relinked.unresolvedPaths).toEqual(["23_public-images/missing.avif"])
  })
